...
**v1.10.0** · Three-source skill discovery · 10 supported AI tools · Cross-tool sync status reporter · ChatGPT cloud upload · Hardened security scanning
...
**Supported `--tools` values:** `claude`, `gemini`, `antigravity`, `opencode`, `openclaw`, `cc-claw`, `codex`, `goose`, `roo`, `cursor`, `cline` (`gemini` and `codex` both install to `~/.agents/skills/`)
